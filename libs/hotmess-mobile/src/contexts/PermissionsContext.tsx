...
function getHighestRole(...roles: Array<UserRole | null>): UserRole {
  let highest = -1;
...
export function PermissionsProvider({ children }: { children: ReactNode }) {
  ...
  const fetchPermissions = useCallback(async () => {
    ...
    try {
      // Suppress unused var warning - accessToken will be used for SE roles later
      void getAccessToken;
  ...
  const effectiveRole = useMemo(
    () => getHighestRole(dbHighestRole, contextRole, teamBasedRole),
    [dbHighestRole, contextRole, teamBasedRole]
  );
